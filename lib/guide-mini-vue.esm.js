...
function getShapeFlag(type) {
    return isStr(type) ? 1 /* ELEMENT */ : 4 /* STATEFUL_COMPONENT */;
}
...
function mountChild(vnode, container) {
    vnode.forEach((child) => patch(child, container));
}
    ...
    return {
        mount(rootContainer) {
            ...
            if (!isElement(rootContainer) && isStr(rootContainer)) {
                ...
                console.warn('mount function only element nodes and selectors can be accepted');
            }
            // NOTE
            // 先将组件转换成 vnode
...
export { REACTIVE_FLAGS, ReactiveEffect, computed, createApp, createComponentInstance, createVNode, effect, h, isProxy, isReactive, isReadOnly, isRef, isTracking, isVNode, proxyRefs, reactive, readonly, ref, render, setupComponent, shallowReadonly, stop, track, trackEffects, trigger, triggerEffects, unRef };
